...
async function generateVariants(
  ...
  for (const variant of VARIANTS) {
    ...
    const publicUrl = `/gallery-success/_processed/${filename}`;
...
async function processImage(filePath: string, relativePath: string, outputDir: string) {
  ...
  console.log(`Processing success story: ${filename} -> ${slug}`);
...
async function main() {
  try {
    const galleryDir = path.resolve(__dirname, '../public/gallery-success');
    const outputDir = path.resolve(__dirname, '../public/gallery-success/_processed');
    
    console.log('📸 Starting success stories gallery ETL process...');
    ...
    console.log(`\n🎉 Success stories gallery ETL completed!`);
