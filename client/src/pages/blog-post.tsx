    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-800 mb-4">{t('blog.not.found')}</h1>
          <p className="text-gray-600 mb-8">{t('blog.not.found.description')}</p>
          ...
          <div className={`transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
            ...
            <div className="flex items-center space-x-6 text-gray-600 mb-8">
              ...
              <div className="flex items-center">
                ...
                <span>{Math.ceil(post.content.split(' ').length / 200)} {t('blog.min.read')}</span>
              </div>
            </div>
          ...
          <article className="lg:col-span-3">
            <div className={`bg-white rounded-2xl shadow-lg p-8 md:p-12 transition-all duration-1000 delay-300 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
              ...
              <div className="mt-12 p-8 bg-gradient-to-r from-[var(--premium-accent)]/10 to-[var(--golden-bronze)]/10 rounded-xl border border-[var(--premium-accent)]/20">
                <h3 className="text-2xl font-bold mb-4 text-gray-800">{t('blog.ready.to.start')}</h3>
                <p className="text-gray-600 mb-6">{t('blog.ready.description')}</p>
                <div className="flex flex-col sm:flex-row gap-4">
                  <Link href="/contact" className="inline-flex items-center justify-center px-6 py-3 bg-[var(--premium-accent)] text-white rounded-full hover:bg-[var(--golden-bronze)] transition-all transform hover:scale-105">
                    {t('blog.start.journey')}
                  </Link>
          ...
          <aside className="lg:col-span-1">
            <div className={`sticky top-32 transition-all duration-1000 delay-500 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
              <div className="bg-white rounded-2xl shadow-lg p-6 mb-8">
                <h3 className="text-xl font-bold mb-6 text-gray-800">{t('blog.related.articles')}</h3>
                    ...
                    >
                      <div className="border border-gray-200 rounded-xl p-4 hover:border-[var(--premium-accent)] hover:shadow-md transition-all">
                        ...
                        <h4 className="font-semibold text-gray-800 group-hover:text-[var(--premium-accent)] transition-colors line-clamp-2 mb-2">
                          {t(`blog.post.${relatedPost.slug}.title`) || relatedPost.title}
                        </h4>
                        {relatedPost.tag && (
                          <span className="inline-block px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded-full">
                            {t(`blog.post.${relatedPost.slug}.tag`) || relatedPost.tag}
                          </span>
                        )}
                      </div>
                    </Link>
              ...
              <div className="bg-gradient-to-br from-[var(--premium-accent)]/10 to-[var(--golden-bronze)]/10 rounded-2xl p-6 border border-[var(--premium-accent)]/20">
                <h3 className="text-lg font-bold mb-4 text-gray-800">{t('blog.quick.links')}</h3>
                <div className="space-y-3">
                  ...
                  <Link href="/instructors" className="block text-gray-600 hover:text-[var(--premium-accent)] transition-colors">
                    {t('blog.meet.instructors')}
                  </Link>
                  <Link href="/gallery" className="block text-gray-600 hover:text-[var(--premium-accent)] transition-colors">
                    {t('blog.student.gallery')}
                  </Link>
                  <Link href="/contact" className="block text-gray-600 hover:text-[var(--premium-accent)] transition-colors">
                    {t('blog.contact.us')}
                  </Link>
                </div>
              </div>
